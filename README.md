...
[万象词库与万象语言模型](https://github.com/amzxyz/RIME-LMDG) 是一个为拼音输入精心打造的工具库。项目提供的带声调的词库，经过AI和大基数语料筛选、加频，结合语法模型获得更准确的首选输出、整句输出。还提供了拼音加注工具链、辅助码刷新、语法模型构建等一系列基础设施，项目是唯一一个死磕分词结构、词序、拼音正确性的库，拥有手工修订的最全面的拼音加注元数据库，打造最强基础底座。

万象拼音输入方案就基于这个底座打造，结合了中英文混输，一套词库，多种用法，开发了众多Lua插件来配合增强体验。

### 优势

1. 词库词语全部加音调
2. 设计汇集7种辅助码，头部使用全拼编码，可以转化为任何双拼编码
  - 仓库版本解码顺序为：全拼拼音；墨奇码；鹤形；自然码；简单鹤；虎码首末；五笔前2；汉心码
  - 因此，万象拼音支持拼音和辅助码任意两两组合
  - release版本则做了分包处理，提供对应的单一辅助码方案包下载，也是我们日常使用的版本
...
**功能3**  通过 拼音状态下输入``` ``〔反查：部件|笔画〕``` 来引导拆字模式 举例 `震`  假设你不认识，你可以通过`雨和辰` 来合并输入，拼音状态输入后，继续输入其它字符字母``` `` ``` 引导符号会消失如下图，输入 `yu if` 即雨 辰，结果出现了我们要的震字，且给出了辅助码 `y` 和  `i`  ，`y`是雨的声母`y`，`i`是辰的声母`ch`，同时通过hspnz分别代表横竖撇捺折，两种类型兼容。
